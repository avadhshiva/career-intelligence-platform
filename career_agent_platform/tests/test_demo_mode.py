...
def test_review_queue_skips_writes_in_demo_mode(demo_env: None, tmp_path: Path) -> None:
    ...
    posting = parser.parse_pasted_text(JD_TPM, job_id="demo_q", title="TPM", company="Co")
    _, recs = RecommendationEngine().recommend_from_resume(RESUME_TPM, [posting])
