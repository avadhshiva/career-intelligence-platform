...
def analyze_capability_gaps(
    ...
    for dim, gap, target_weight in gaps[:8]:
        ...
        if hint and len(evidence) < 5:
            evidence.append(
                f"Surface quantified outcomes for {hint} in a top-third resume bullet."
            )

    family_hint = ROLE_FAMILY_RESUME_HINT.get(target_family)
    if family_hint:
        strengthening.append(family_hint)

    if gaps:
        top_dim, _, target_weight = gaps[0]
        strategic = phrase_gap(
            top_dim,
            float(candidate_vector.get(top_dim, 0.0)),
            target_weight,
        )
        strengthening.append(
            f"To close the gap: {strategic.rstrip('.')} — add a bullet with scope, "
            f"stakeholders, and measurable outcome."
        )

    priority_dims = [d for d, _, _ in gaps[:3]]
    if "ai_strategy" in priority_dims or "transformation_strategy" in priority_dims:
        strengthening.append(
            "Lead the professional summary with transformation or AI portfolio "
            f"ownership before delivery mechanics for {display} roles."
        )
