...
class ResumeParseError(ValueError):
    """Deterministic resume extraction or parsing failure."""
...
class ResumeParser:
    ...
    def parse_bytes(self, content: bytes, filename: str) -> ParsedResume:
        ...
        if not content:
            raise ResumeParseError("Resume file is empty.", reason="empty_extraction")
        return self.parse_text(content.decode("utf-8", errors="replace"))

    def parse_pdf(self, content: bytes) -> ParsedResume:
        if not content:
            raise ResumeParseError("Resume file is empty.", reason="empty_extraction")
        try:
            raw = self._pdf_to_text(content)
        except Exception as exc:
            raise ResumeParseError(
                f"Corrupted or unreadable PDF: {exc}",
        ...
        return self._parse_text_or_fail(raw)

    def parse_docx(self, content: bytes) -> ParsedResume:
        if not content:
            raise ResumeParseError("Resume file is empty.", reason="empty_extraction")
        try:
            raw = self._docx_to_text(content)
        except Exception as exc:
            raise ResumeParseError(
    ...
    def parse_text(self, raw_text: str) -> ParsedResume:
        ...
        if not text:
            raise ResumeParseError(
                "Resume text is empty after normalization.",
                reason="empty_extraction",
            )
        return parsed

    def _parse_text_or_fail(self, raw_text: str) -> ParsedResume:
        try:
            ...
            raise ResumeParseError(
                f"Failed to parse resume text: {exc}",
                reason="parsing_failure",
            ) from exc
